...
def pixel2cam(depth, pixel_coords, intrinsics, is_homogeneous=False):
    ...
    print(pixel_coords)
    #sys.exit()
    print("intrinsics invert =", intrinsics.shape)
    print(intrinsics)
    #sys.exit()
    #new_inv = tf.linalg.inv(intrinsics)
    #print("sdf",new_inv)
...
def cam2pixel(cam_coords, proj):
    ...
    unnormalized_pixel_coords = tf.reshape(unnormalized_pixel_coords, [batch, 3, -1])     #Added by me to adjust dimentions.
    #print(unnormalized_pixel_coords.shape)
   # print("height is now")
    #print(height, width, batch)
    ...
    new_height, new_width = 224, 384
    pixel_coords = tf.concat([x_n, y_n], axis=1)
   # print (pixel_coords.shape)
    #pixel_coords = tf.image.resize(pixel_coords, [new_height, new_width])
    pixel_coords = tf.reshape(pixel_coords, [batch, 2, new_height, new_width])
    # pixel_coords = tf.transpose(pixel_coords, perm=[0, 2, 3, 1])  # Adjust the transpose
