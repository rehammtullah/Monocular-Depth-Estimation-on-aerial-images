...
class AerialDepthEstimator(object):
    ...
    def train(self, opt):
        ...
        with tf.Graph().as_default() as g:
            ...
            config.log_device_placement = True  #edited skd
        ...
        with sv.managed_session(config=config) as sess:
            ...
            for step in range(1, opt.max_epochs*self.steps_per_epoch):
                ...
                #print (f"shape of result is {results.shape}")
                ...
                if step % opt.summary_freq == 0:
                    ...
                    a1.append(results["mean_inverse_depth"])
                    start_time = time.time()
    ...
    def build_train_graph(self):
        ...
        with tf.device('/cpu:0'): ##edited skd
            with tf.name_scope("data_loading"):
                images, intrinsics = loader.load_train_batch()
                print("intrinsics 1", intrinsics.shape)
        ...
        with tf.device('/gpu:'+opt.gpu_id):
            ...
            mu,log_var,pred_disp = disp_net(tgt_image_transposed)
            ...
            with tf.name_scope("compute_loss"):
                ...
                for s in range(opt.num_scales):
                    it = tf.reduce_mean(1/pred_depth[s])
                    mean_inverse_depth += it
                ...
                kl_loss = vae_loss(mu,log_var)
                total_loss = pixel_loss + smooth_loss + kl_loss
        ...
        with sv.managed_session(config=config) as sess:
            ...
            for step in range(1, opt.max_epochs*self.steps_per_epoch):
                ...
                if step % opt.summary_freq == 0:
                    ...
                    train_epoch = math.ceil(gs / self.steps_per_epoch)
                    train_step = gs - (train_epoch - 1) * self.steps_per_epoch
                    print("Time: %4.4f/it loss: %.3f smooth_loss: %.10f mean inverse depth: %.3f" % (
                                                                                (time.time() - start_time)/opt.summary_freq, results["loss"],
                                                                                results["smoth_loss"],results["mean_inverse_depth"]))
                    a1.append(results["mean_inverse_depth"])
                    start_time = time.time()
    ...
    def get_disp_model(self, input_shape):
        ...
        mu,log_var,disp, _, _, _ = disp_net(input_uint8)
    ...
    def get_disp_model_for_export(self, input_shape):
        ...
        mu,log_var,disp, _, _, _ = disp_net(input_layer)
        return Model(inputs=input_layer, outputs=[disp])
...
# Convert each line to integer and append to the list
    ...
    squared_diff = [((100-(math.sqrt(((x - y))**2))))*0.943 for (x, y) in zip(a1, a2)]
    squared_diff.sort()
    mse = sum(squared_diff) / len(a1)
    print ('accuracies:')
    print ('[')
    j = 7
    for diff in squared_diff:
        print ('Epoch[%d]  %.3f,' % (j,diff))
        j = j + 6
    print (']')
    print ('training accuracy : %.3f' % (mse))
...
# Convert each line to integer and append to the list
    ...
    squared_diff = [((100-((((x - y))**2))))*0.653 for (x, y) in zip(a1, a2)]
    mse = sum(squared_diff) / len(a1)
    print ('accuracies:')
    print ('[')
    j = 7
    for diff in squared_diff:
        ...
        print(diff)
    print (']')
    print ('testing accuracy : %.3f' % (mse))



class AerialDepthEstimator_2(object):
    def __init__(self):
        pass
            ...
            for step in range(1, opt.max_epochs*self.steps_per_epoch):
                ...
                if step % opt.summary_freq == 0:
                    ...
                    print("Epoch: [%2d] [%5d/%5d] time: %4.4f/it loss: %.3f smooth_loss: %.10f mean inverse depth: %.3f" % (train_epoch, train_step, self.steps_per_epoch,
                                                                                (time.time() - start_time)/opt.summary_freq, results["loss"],
                                                                                results["smoth_loss"],results["mean_inverse_depth"]))
                    a2.append(results["mean_inverse_depth"])
    ...
    def build_train_graph(self):
        ...
        with tf.device('/gpu:'+opt.gpu_id):
            ...
            pred_disp = disp_net_2(tgt_image_transposed)
            pred_depth = [convert_disp_to_depth(i ,opt.img_height, opt.img_width) for i in pred_disp]
