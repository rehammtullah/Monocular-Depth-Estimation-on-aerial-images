...
def build_res_net_encoder18(inputs, name):
    ...
    mu = res_block(econv4,512,"res_4")
    log_var = res_block(econv4,512,"res_5")
    


    return Model(inputs=input_layer, outputs=[mu, log_var, econv1, econv2, econv3, econv4, econv5])
...
def pose_net_new(image_1, image_2):
    # ENCODING
    mu1,log_var1,_,_,_,_,deph_feature_map_image_1 = shared_encoder(image_1)(image_1)
    mu2,log_var2,_,_,_,_,deph_feature_map_image_2 = shared_encoder(image_2)(image_2)
...
def pose_net(tgt_image, src_image_stack, num_source):
    # ENCODING
    mu,log_var,_,_,_,_,deph_feature_map_tgt_image = shared_encoder(tgt_image)(tgt_image)

    deph_feature_src_maps=[]
    for i in range(num_source):    
        src_image = src_image_stack[:, :, :, i*3:(i+1)*3]
        mu,log_var ,_,_,_,_,deph_feature_map_src_image = shared_encoder(src_image)(src_image)
...
def pose_net_old(src_image1, tgt_image, src_image2):
    # ENCODING
    mu1,log_var1,_, _, _, _, deph_feature_map_src_image1 = shared_encoder(src_image1)(src_image1)
    mu2,log_var2,_, _, _, _, deph_feature_map_tgt_image = shared_encoder(tgt_image)(tgt_image)
    mu3,log_var3,_, _, _, _, deph_feature_map_src_image2 = shared_encoder(src_image2)(src_image2)
    name = "pose_prediction"
    with tf.name_scope(name):
        # DECODING
        num_source = 2
...
def disp_net(input_image):
    # ENCODING
    mu, log_var, skip1, skip2, skip3, skip4, econv5 = shared_encoder(input_image)(input_image)
    
    # Sample from the latent space
    latent_vector = sampling([mu, log_var])
    name = "depth_prediction"
    with tf.name_scope("depth_prediction"):
        # DECODING
        with tf.name_scope('decoder'):
            # desired_shape = econv5.shape[1:]
            # latent_vector_reshaped = tf.reshape(latent_vector, desired_shape)
            filter = [int(skip4.shape[-1]), int(skip3.shape[-1]), int(skip2.shape[-1]), int(skip1.shape[-1]), 16]

            # Start decoding with the sampled latent vector
            upconv5 = up_block(latent_vector, filter[0], name+"_upconv5", skip4)
        ...
        return mu, log_var, [disp1, disp2, disp3, disp4]


def disp_net_2(input_image):
    # ENCODING
    mu, log_var, skip1, skip2, skip3, skip4, econv5 = shared_encoder(input_image)(input_image)
