...
class aerial_raw_loader(object):
    def __init__(self, dataset_dir, img_height=256, img_width=256, seq_length=5, frame_offset=1):
            ...
            if(os.path.isdir(os.path.join(self.dataset_dir + '/'+dir_name))):
                ...
                for seq_name in seq_set:
                    if(os.path.isdir(os.path.join(self.dataset_dir + '/'+dir_name+"/"+seq_name))):
                        ...
                        for n in range(N):                            
                            frame_id = '%.10d' % (900+ 5*(n+1))
                            # print(dir_name + ' ' + seq_name + ' ' + frame_id)
    ...
    def load_image_sequence(self, frames, tgt_idx, seq_length):
        ...
        for o in range(-half_offset, half_offset + 1):
            ...
            curr_img = self.load_image_raw(dir_name, seq_name, '%.10d' % (int(curr_frame_id)+5*(self.frame_offset*half_offset+o*self.frame_offset)))
            if o == 0:
    ...
    def load_image_raw(self, dir_name, seq_name, frame_id):
        img_file = os.path.join(self.dataset_dir, dir_name, seq_name, str(frame_id) + '.jpg')
        fh = open(img_file, 'rb')
        img = pil.open(fh)
        img=np.array(img)
        if img.shape[2]>3:
